...
import { Phone, AlertTriangle, Syringe, Clock, Activity, Heart, Droplet, CheckCircle, XCircle, LogOut, User, Printer, BrainCircuit, Users, PieChart as PieIcon } from 'lucide-react';
...
export default function Dashboard() {
  ...
  // Health Score
  const healthScore = Math.max(0, Math.floor(100 - (Math.abs(vitals.bpSystolic - 120) * 0.4) - (Math.abs(vitals.sugar - 100) * 0.3) - (Math.abs(vitals.heartRate - 75) * 0.3)));
  ...
  // Prediction Engine (Average of last 5 values)
  const predictedValue = graphData.length > 5 
    ? Math.round(graphData.slice(-5).reduce((acc, curr) => acc + curr[activeDetails.dataKey], 0) / 5) 
    : vitals.bpSystolic;

  // Pie Chart Dynamic Data
  const pieData = [
    { name: 'Healthy', value: healthScore > 80 ? 60 : healthScore > 50 ? 40 : 10, color: '#10b981' },
    { name: 'Risk', value: healthScore > 80 ? 30 : healthScore > 50 ? 50 : 30, color: '#f59e0b' },
    { name: 'Critical', value: healthScore > 80 ? 10 : healthScore > 50 ? 10 : 60, color: '#ef4444' },
  ];
    ...
    <>
      ...
      <div className={`dashboard-container ${isEmergency ? 'emergency-content' : ''}`}>
        ...
        <div className="metrics-area">
          <div className={`metric-card ${activeTab === 'bp' ? 'active' : ''}`} onClick={() => setActiveTab('bp')}>
          ...
          <div className={`metric-card ${activeTab === 'sugar' ? 'active' : ''}`} onClick={() => setActiveTab('sugar')}>
            <div className="metric-header">
          ...
          <div className={`metric-card ${activeTab === 'hr' ? 'active' : ''}`} onClick={() => setActiveTab('hr')}>
            <div className="metric-header">
          ...
          <div className={`metric-card ${activeTab === 'oxygen' ? 'active' : ''}`} onClick={() => setActiveTab('oxygen')}>
            <div className="metric-header">
        ...
        <div className="main-content-grid">
          <div className="glass-panel graph-section">
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start'}}>
              <div>
                <h3>{activeDetails.name} Trend & Forecast</h3>
                <p className="subtitle" style={{marginBottom: '5px'}}>Real-time monitoring</p>
                <p className="subtitle" style={{color: '#94a3b8', fontSize: '12px'}}>
                  <BrainCircuit size={12} style={{display:'inline', verticalAlign:'middle'}}/> AI Prediction (Next 3h): <b style={{color: '#3b82f6'}}>{predictedValue} {activeDetails.unit}</b>
                </p>
              </div>
              <div style={{fontWeight: 700, color: activeDetails.color}}>{activeDetails.risk}</div>
            </div>
              ...
              <AreaChart data={graphData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <defs>
                  <linearGradient id="colorValue" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={activeDetails.fill} stopOpacity={0.6}/>
                    <stop offset="95%" stopColor={activeDetails.fill} stopOpacity={0}/>
                  </linearGradient>
                </defs>
                ...
                <Area type="monotone" dataKey={activeDetails.dataKey} stroke={activeDetails.stroke} strokeWidth={3} fillOpacity={1} fill="url(#colorValue)" name={activeDetails.name} activeDot={{r: 8}} />
              </AreaChart>
            </ResponsiveContainer>
          ...
          <div className="side-panels">
            {/* Pie Chart Component */}
            <div className="glass-panel" style={{marginBottom: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
              <h3 style={{alignSelf: 'flex-start', display:'flex', gap:'8px', alignItems:'center'}}><PieIcon size={18} color="#3b82f6"/> Health Distribution</h3>
              ...
              <div style={{display:'flex', gap:'15px', fontSize:'12px', fontWeight:600, marginTop:'-10px'}}>
